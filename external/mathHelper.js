...
function random3DVec(center, spreadVec) {
  return { x: randomSpread(center.x, spreadVec.x), y: randomSpread(center.y, spreadVec.y), z: 0 };
}
...
function randomUnitVec3D(center, spread) {
  angle = randomSpread(center, spread);
  // return { x: Math.cos(angle), y: Math.sin(angle), z: Math.sin(angle) };
  return { x: Math.cos(angle) * Math.sin(Math.acos(randomSpread(0, 1))), y: Math.sin(angle) * Math.sin(Math.acos(randomSpread(0, 1))), z: 0.0 };
}
...
function addVec3Ds(vec1, vec2) {
  return { x: vec1.x + vec2.x, y: vec1.y + vec2.y, z: 0 };
}
...
function scaleVec3D(vec, scalar) {
  return { x: vec.x * scalar, y: vec.y * scalar, z: 0 };
}
