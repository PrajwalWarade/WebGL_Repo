...
vec3 getIBLContribution(MaterialInfo materialInfo, vec3 n, vec3 v) {
    ...
    //vec3 diffuseLight = srgbToLinear(texture(uEnvironmentDiffuse, n)).rgb * 0.1; TODO
    //vec3 specularLight = srgbToLinear(texture(uEnvironmentSpecular, n)).rgb * 0.2; TODO
...
void main() {
    ...
    //FragColor = vec4(materialInfo.diffuseColor, 1.0);
    //FragColor = baseColor;

    vec3 lit = color;  
    lit = lit / (lit + vec3(1.0));
    FragColor = vec4(pow(lit, vec3(1.0 / 2.2)), baseColor.a);
}
